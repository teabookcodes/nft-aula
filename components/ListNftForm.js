...
export default function ListNftForm() {
  ...
  function submitForm() {
    ...
    if (collectionWebsite) {
      if (!linkRegex.test(collectionWebsite)) {
        ...
        /^(https?:\/\/)?(twitter\.com)[^\s]+$/.test(collectionWebsite)
      ) {
  ...
  return (
    <form onSubmit={handleSubmit} className="text-gray-700">
      ...
      <div className="mb-6">
        <p className="text-sm text-aulaBlack">
          * Values marked with color are required
        </p>
      </div>
  ...
  // return <p>Don&apos;t worry, you will soon be able to upload NFTs as usual</p>;
}
